...
# Finally it generates a binning file for SO with format : bin_min, bin_max, bin_mean.
...
#+RESULTS:
[[file:./.ob-jupyter/9f2767a6702f739f35d7822ef027e4d6cb6a6407.png]]
...
#+RESULTS:
[[file:./.ob-jupyter/9f6d3f761068a1abd031e1e4f521f0e1ac07a750.png]]
...
#+RESULTS:
:RESULTS:
: <ipython-input-30-0b376b43796a>:17: RuntimeWarning: divide by zero encountered in log10
:   im = ax.imshow(np.log10(np.abs(mcm)), vmin=vmin, vmax=vmax)
[[file:./.ob-jupyter/646f1f48eeae2acfd40ff7826ba0ea39bba10c2d.png]]
[[file:./.ob-jupyter/90f0ce2937717b2899374152f00b86e171a2b1ae.png]]
[[file:./.ob-jupyter/2d6e377dae1f472fea3a5159910eaf92a546808b.png]]
:END:
...
#+RESULTS:
[[file:./.ob-jupyter/2d2c280505ea2288a615600e7434a07c1b059e17.png]]
...
We have to generate realistic simulations by adding the foregrounds and noise to CMB data/alms
splits. Foregrounds are only added for temperature (so far).

We also have to "blur" the simulation given the beam computed in previous section.
#+BEGIN_SRC jupyter-python
  from pspy import so_map
  ...
  for i, freq in enumerate(frequencies["LAT"]):
    ...
    #   maps["LAT", freq, k] = sph_tools.alm2map(split_alms, template)
#+END_SRC

#+RESULTS:

#+BEGIN_SRC jupyter-python
  fig, axes = plt.subplots(nrows=len(maps)+1, ncols=3, figsize=(15, 21))
  axes = axes.flatten()
  cmb_map = sph_tools.alm2map(cmb_alms, template)
  for i, spec in enumerate(["T", "E", "B"]):
      plt.axes(axes[i])
      hp.mollview(cmb_map.data[i], title=spec, hold=True)
  for i, (k, v) in enumerate(maps.items()):
    for j in range(3):
        plt.axes(axes[3*i+j+3])
        hp.mollview(v.data[j], title="{} GHz - split {}".format(k[1], k[-1]), hold=True)
#+END_SRC

#+RESULTS:
[[file:./.ob-jupyter/e44741bb60f87f8a52c5853e83d98ec20f554857.png]]

#+BEGIN_SRC jupyter-python
  print(id(cmb_map.data[0]))
  print(id(maps["LAT", 93, 0].data[0]))
#+END_SRC
